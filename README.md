...
A macOS menu bar app that keeps your Apple Mail signature fresh: it composes
...
one signature inside Mail (named **Dynamic Quote** by default) via Apple's
scripting interface. You select that signature for your account once; the app
keeps its content rotating from then on.
...
1. Open **Settings → Identity** from the menu bar icon and enter at least
   your name.
2. Click **New Quote Now**. macOS will ask for permission to control Mail —
   ...
   Security → Automation.)
3. In **Mail → Settings → Signatures**, drag the **Dynamic Quote** signature
   onto your account and choose it as the default (or pick it in the
   Signature popup when composing).
...
(`quotes.json`, `identity.json`, `rotation-state.json`).
